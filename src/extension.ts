...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerTextEditorCommand(CAT_NAMES_COMMAND_ID, async (textEditor: vscode.TextEditor) => {
            ...
            const extConfig = workspace.getConfiguration('chat-sample');
            const commands : { id : string, description : string, prompt : string }[] = extConfig.cat.customCommands;
            
            if (commands && commands.length > 0)
            {
                const picked = await vscode.window.showQuickPick(commands.map(c => c.id), { placeHolder: 'Select a custom command to insert in the editor' });
